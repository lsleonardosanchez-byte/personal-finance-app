  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
        ...
        <h2 style={{ color: '#1e293b', fontWeight: 700 }}>{t.admin}</h2>
      </div>
      ...
      <div style={{ background: 'white', borderRadius: '12px', padding: '1.5rem', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
        ...
        {loading ? (
          <p style={{ color: '#94a3b8', textAlign: 'center', padding: '2rem 0' }}>{t.loadingUsers}</p>
        ) : users.length === 0 ? (
          <p style={{ color: '#94a3b8', textAlign: 'center', padding: '2rem 0' }}>{t.noUsers}</p>
        ) : (
            ...
            {users.map(user => (
              <div key={user.id} style={{
                ...
                {!user.isAdmin && (
                  ...
                  >
                    {resetSuccess === user.id ? t.pinResetSuccess : t.resetPin}
                  </button>
                )}
              </div>
            ))}
