...
export default function FixedExpenses({ userId, categories, onTransactionChange }) {
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto' }}>
      ...
      <p style={{ color: '#94a3b8', fontSize: '0.85rem', marginBottom: '1.5rem' }}>Resets automatically every 1st of the month</p>
              ...
              <div key={f.id} style={{
                ...
                {!isEditing && (
                  <div style={{ padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      ...
                      {!isPaid && (
                        <button onClick={() => startEdit(f)} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '6px', padding: '0.3rem 0.6rem', fontSize: '0.8rem', cursor: 'pointer', color: '#6366f1', fontWeight: 500 }}>Edit</button>
                      )}
                ...
                {isEditing && (
                  <div style={{ padding: '1rem 1.25rem', background: '#fffbeb' }}>
                    <p style={{ fontSize: '0.85rem', color: '#f59e0b', fontWeight: 500, marginBottom: '0.75rem' }}>Editing fixed expense:</p>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginBottom: '0.75rem' }}>
                      <div style={{ gridColumn: '1 / -1' }}>
                        <label style={{ fontSize: '0.75rem', color: '#64748b', fontWeight: 500, display: 'block', marginBottom: '0.2rem' }}>Name</label>
                        <input type="text" value={editValues.name} onChange={e => setEditValues(p => ({ ...p, name: e.target.value }))} style={editInputStyle} />
                      </div>
                      <div>
                        <label style={{ fontSize: '0.75rem', color: '#64748b', fontWeight: 500, display: 'block', marginBottom: '0.2rem' }}>Default Amount (COP)</label>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      <button onClick={() => saveEdit(f.id)} style={{ flex: 1, padding: '0.6rem', background: '#6366f1', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 600, cursor: 'pointer' }}>Save</button>
                ...
                {isPickingDate && !isEditing && (
                  <div style={{ padding: '0.75rem 1.25rem 1rem', borderTop: '1px solid #f1f5f9', background: '#f8fafc' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginBottom: '0.75rem' }}>
                      ...
                      <div>
                        <label style={{ fontSize: '0.75rem', color: '#64748b', fontWeight: 500, display: 'block', marginBottom: '0.3rem' }}>Amount (COP)</label>
                        <input type="number" value={payAmount} onChange={e => setPayAmount(e.target.value)} style={{ ...inputStyle, width: '100%' }} />
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      <button onClick={() => confirmPaid(f)} style={{ flex: 1, padding: '0.6rem', background: '#22c55e', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 600, cursor: 'pointer' }}>Confirm Payment</button>
                      <button onClick={() => { setPendingPay(null); setPayAmount(''); }} style={{ padding: '0.6rem 1rem', background: '#f1f5f9', color: '#64748b', border: 'none', borderRadius: '8px', fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
                    </div>
                  </div>
                )}
              </div>
